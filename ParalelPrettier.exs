defmodule Syntaxhighlighter do
  def highlight(directory_path) do
    ...
    |> Enum.each(fn
      ...
      {:exit, reason} -> IO.puts("Error processing file: #{inspect(reason)}")
    end)
  end
  ...
  defp highlight_file(file) do
    ...
    case File.read(expanded_path) do
      {:ok, text} ->
        IO.puts("File successfully read: #{expanded_path}")
        ...
        File.write(
          "#{Path.dirname(expanded_path)}/#{Path.basename(expanded_path, Path.extname(expanded_path))}.html",
          html_content
        )

      {:error, reason} ->
        IO.puts("Failed to read file: #{expanded_path}. Reason: #{inspect(reason)}")
        {:error, "Failed to read file: #{expanded_path}. Reason: #{inspect(reason)}"}
    end
  end
